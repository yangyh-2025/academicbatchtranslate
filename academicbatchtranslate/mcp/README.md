...
```json
{
  "mcpServers": {
    "academicbatchtranslate": {
      "command": "uvx",
      "args": ["--from", "academicbatchtranslate[mcp]", "academicbatchtranslate", "--mcp"],
      "env": {
        "ACADEMICBATCHTRANSLATE_API_KEY": "sk-xxxxxx",
        "ACADEMICBATCHTRANSLATE_BASE_URL": "https://api.openai.com/v1",
        "ACADEMICBATCHTRANSLATE_MODEL_ID": "gpt-4o",
        "ACADEMICBATCHTRANSLATE_TO_LANG": "Chinese",
        "ACADEMICBATCHTRANSLATE_CONCURRENT": "10",
        "ACADEMICBATCHTRANSLATE_CONVERT_ENGINE": "mineru",
        "ACADEMICBATCHTRANSLATE_MINERU_TOKEN": "your-mineru-token"
      }
    }
  }
}
...
```json
{
  "mcpServers": {
    "academicbatchtranslate": {
      ...
      "args": ["-m", "academicbatchtranslate.mcp"],
      "env": {
        "ACADEMICBATCHTRANSLATE_API_KEY": "sk-xxxxxx",
        "ACADEMICBATCHTRANSLATE_BASE_URL": "https://api.openai.com/v1",
        "ACADEMICBATCHTRANSLATE_MODEL_ID": "gpt-4o",
...
Or if you have `academicbatchtranslate` available in PATH:

```json
{
  "mcpServers": {
    "academicbatchtranslate": {
      "command": "academicbatchtranslate",
      "args": ["--mcp"],
      "env": {
        "ACADEMICBATCHTRANSLATE_API_KEY": "sk-xxxxxx",
        "ACADEMICBATCHTRANSLATE_BASE_URL": "https://api.openai.com/v1",
...
```bash
academicbatchtranslate --mcp --transport sse --mcp-host 127.0.0.1 --mcp-port 8000
```

Or using the module directly with virtual environment Python:

```bash
/path/to/your/venv/bin/python -m academicbatchtranslate.mcp --transport sse --host 127.0.0.1 --port 8000
```
...
```bash
academicbatchtranslate --mcp --transport streamable-http --mcp-host 127.0.0.1 --mcp-port 8000
```
...
```bash
academicbatchtranslate -i --with-mcp
```
...
|---------------------|-------------|----------|
| `ACADEMICBATCHTRANSLATE_API_KEY` | AI platform API key | Yes |
| `ACADEMICBATCHTRANSLATE_BASE_URL` | AI platform base URL | Yes |
| `ACADEMICBATCHTRANSLATE_MODEL_ID` | Model ID | Yes |
| `ACADEMICBATCHTRANSLATE_TO_LANG` | Target language (default: Chinese) | No |
| `ACADEMICBATCHTRANSLATE_CONCURRENT` | Concurrent requests (default: 10) | No |
| `ACADEMICBATCHTRANSLATE_CONVERT_ENGINE` | PDF conversion engine | No |
| `ACADEMICBATCHTRANSLATE_MINERU_TOKEN` | MinerU API Token | No |
...
**Solution:**
1. You can configure via environment variables `ACADEMICBATCHTRANSLATE_API_KEY`, `ACADEMICBATCHTRANSLATE_BASE_URL`, `ACADEMICBATCHTRANSLATE_MODEL_ID`
...
**Solution:**
1. Check if `academicbatchtranslate` command is in PATH
  ...
  "mcpServers": {
    "academicbatchtranslate": {
      ...
      "env": {
        ...
        "ACADEMICBATCHTRANSLATE_BASE_URL": "https://api.openai.com/v1",
        "ACADEMICBATCHTRANSLATE_MODEL_ID": "gpt-4o",
        "ACADEMICBATCHTRANSLATE_TO_LANG": "中文",
        "ACADEMICBATCHTRANSLATE_CONCURRENT": "10",
        "ACADEMICBATCHTRANSLATE_CONVERT_ENGINE": "mineru",
        "ACADEMICBATCHTRANSLATE_MINERU_TOKEN": "your-mineru-token"
      }
...
如果 PATH 中已有 `academicbatchtranslate`，也可以使用：
...
| 环境变量 | 说明 | 必需 |
|----------|------|------|
| `ACADEMICBATCHTRANSLATE_API_KEY` | AI 平台 API 密钥 | 是 |
| `ACADEMICBATCHTRANSLATE_BASE_URL` | AI 平台基础 URL | 是 |
| `ACADEMICBATCHTRANSLATE_MODEL_ID` | 模型 ID | 是 |
| `ACADEMICBATCHTRANSLATE_TO_LANG` | 目标语言（默认：中文） | 否 |
| `ACADEMICBATCHTRANSLATE_CONCURRENT` | 并发请求数（默认：10） | 否 |
| `ACADEMICBATCHTRANSLATE_CONVERT_ENGINE` | PDF 转换引擎 | 否 |
| `ACADEMICBATCHTRANSLATE_MINERU_TOKEN` | MinerU API Token | 否 |
...
**解决：**
1. 可以通过设置环境变量 `ACADEMICBATCHTRANSLATE_API_KEY`、`ACADEMICBATCHTRANSLATE_BASE_URL`、`ACADEMICBATCHTRANSLATE_MODEL_ID` 来配置
...
**解决：**
1. 检查 `academicbatchtranslate` 命令是否在 PATH 中
2. 尝试使用虚拟环境的绝对路径
3. 重启 Claude Desktop
